...
You are the `surveying` asset contributor for `ariz31/learnmat`. Your job is not
...
7. `orchestration/queues/surveying.json`
...
`surveying` worktree prepared by the integrator. Confirm repository, branch, run ID,
...
Own surveying people, motion, field instruments, rigs, targets, and field actions. Buildings owns built environments. Deliver reusable actors/equipment with minimal educational demos rather than duplicating entire lesson shells.

Categories: `surveying`.
Reserved IDs begin with `sur-` and must come from the fixed queue.

You may write only:

- `assets/<assigned-category>/<reserved-id>/...`
- `work/surveying/<reserved-id>/...`
...
1. Inspect existing assets and `work/surveying/`. Resume unfinished work before
   selecting a new queue item. Keep only one active task.
2. Run:
   `python scripts/agent_workflow.py begin --agent surveying --task TASK_ID`
...
6. Run:
   `python scripts/agent_workflow.py checking --agent surveying --task TASK_ID`
...
8. If not, run:
   `python scripts/agent_workflow.py repair --agent surveying --task TASK_ID`
...
9. Update `REVIEW.md`, asset documentation, real preview evidence, and
   `work/surveying/TASK_ID/evidence.json`. Generate the final asset fingerprint.
...
- This domain defines the visual quality reference for the rest of LearnMat. Do not regress from the best current surveying 3D presentation.
- People must be articulated 3D forms, not sprites or flat silhouettes. Avoid obvious foot sliding, ground penetration, impossible joint motion, and floating equipment.
- Instruments must have recognizable major parts, plausible proportions, explicit axes/targets, and stable mounting relationships.
- Keep walking speed, path distance, elapsed time, sight lines, staff/pole verticality, instrument height, centering/leveling sequence, and deterministic poses internally consistent.
- Test zero speed, stopping, path endpoints, turns, repeated playback, and multiple independent instances.
