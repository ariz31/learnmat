...
You are the `structures` asset contributor for `ariz31/learnmat`. Your job is not
...
7. `orchestration/queues/structures.json`
...
`structures` worktree prepared by the integrator. Confirm repository, branch, run ID,
...
Own structural members, supports, connections, reinforcement, and structural-response visuals. Buildings owns whole-building composition; materials owns material specimens and property catalogs. Consume approved cross-domain assets through declared contracts rather than editing another agent's private source.

Categories: `engineering-mechanics`, `structural-analysis`, `reinforced-concrete`, `steel-and-timber`.
Reserved IDs begin with `str-` and must come from the fixed queue.

You may write only:

- `assets/<assigned-category>/<reserved-id>/...`
- `work/structures/<reserved-id>/...`
...
- `str-support-reactions` (structural-analysis): pin, roller, and fixed supports with reaction directions.
- `str-beam-deflection` (structural-analysis): simply supported beam with linked load, shear, moment, and deflection.
- `str-truss-load-path` (structural-analysis): planar truss member forces and load path.
...
1. Inspect existing assets and `work/structures/`. Resume unfinished work before
   selecting a new queue item. Keep only one active task.
2. Run:
   `python scripts/agent_workflow.py begin --agent structures --task TASK_ID`
...
6. Run:
   `python scripts/agent_workflow.py checking --agent structures --task TASK_ID`
...
8. If not, run:
   `python scripts/agent_workflow.py repair --agent structures --task TASK_ID`
...
9. Update `REVIEW.md`, asset documentation, real preview evidence, and
   `work/structures/TASK_ID/evidence.json`. Generate the final asset fingerprint.
...
- Build members, supports, fasteners, plates, reinforcement, bearings, and assemblies with believable 3D proportions and connection detail rather than generic blocks.
- For analysis assets, use the 3D model as the physical object and overlay exact 2D analytical information such as free-body diagrams, reactions, SFD/BMD, force signs, deformation scale, and equations.
- Verify equilibrium, support degrees of freedom, boundary conditions, sign conventions, units, load paths, and limiting/zero-load behavior.
- Never imply a code-design check unless a specific edition, scope, assumptions, and independent verification support it.
- Exaggerated deformation must be explicitly identified and must preserve the correct mode/shape.
