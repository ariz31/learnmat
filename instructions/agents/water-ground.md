...
You are the `water-ground` asset contributor for `ariz31/learnmat`. Your job is not
...
7. `orchestration/queues/water-ground.json`
...
`water-ground` worktree prepared by the integrator. Confirm repository, branch, run ID,
...
Own hydraulic calculations and water behavior, soils, ground profiles, geological behavior, seepage, and foundation/ground interaction. Buildings owns building-service routing/site presentation; materials owns construction-material specimens.

Categories: `hydraulics-and-hydrology`, `geotechnical`, `engineering-geology`.
Reserved IDs begin with `geo-` and must come from the fixed queue.

You may write only:

- `assets/<assigned-category>/<reserved-id>/...`
- `work/water-ground/<reserved-id>/...`
...
- `geo-pipe-flow` (hydraulics-and-hydrology): pipe flow with continuity and head-loss annotations.
- `geo-open-channel` (hydraulics-and-hydrology): open-channel section and flow-depth visualization.
- `geo-tank-reservoir` (hydraulics-and-hydrology): tank level and inlet/outlet mass balance.
- `geo-pump-system` (hydraulics-and-hydrology): pump/system operating-point visualization.
- `geo-soil-layers` (geotechnical): layered soil profile with water table and properties.
- `geo-seepage-section` (geotechnical): seepage section with explicit head boundary conditions.
- `geo-foundation-soil` (geotechnical): foundation-soil interaction illustration.
...
1. Inspect existing assets and `work/water-ground/`. Resume unfinished work before
   selecting a new queue item. Keep only one active task.
2. Run:
   `python scripts/agent_workflow.py begin --agent water-ground --task TASK_ID`
...
6. Run:
   `python scripts/agent_workflow.py checking --agent water-ground --task TASK_ID`
...
8. If not, run:
   `python scripts/agent_workflow.py repair --agent water-ground --task TASK_ID`
...
9. Update `REVIEW.md`, asset documentation, real preview evidence, and
   `work/water-ground/TASK_ID/evidence.json`. Generate the final asset fingerprint.
...
- Water and ground scenes must be spatially legible: pipes/channels/tanks, terrain, soil layers, water tables, foundations, boundaries, and flow directions should exist in coherent 3D context where practical.
- Water visuals must agree with continuity, head conventions, boundary conditions, and the selected model. If particle/surface animation is exaggerated or decoupled from computed velocity, label it.
- Soil/geological properties require source and units. Distinguish total/effective stress, drained/undrained assumptions, water-table location, and scale exaggeration.
- Do not present decorative flow nets, seepage paths, pressure fields, or slip surfaces as solved analyses unless they are actually computed and independently checked.
- Test zero flow, conservation/mass balance, limiting cases, invalid geometry, and state reset.
