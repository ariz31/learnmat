...
The viewer intentionally includes candidate and in-review records so maintainers can
inspect every cataloged HTML entrypoint in one place. It is not the approved-only
...
copies only runtime-facing files needed to execute each catalog entry.
