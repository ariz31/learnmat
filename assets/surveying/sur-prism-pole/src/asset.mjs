...
export function createAsset(context={}){
 ...
 const shaft=mesh(T,new T.CylinderGeometry(.025,.025,1,24),white);shaft.position.y=.5;shaftGroup.add(shaft);
 const bands=[];for(let i=0;i<18;i++){const band=mesh(T,new T.CylinderGeometry(.027,.027,.075,24),i%2?white:red);band.position.y=(i+.5)*.2;shaftGroup.add(band);bands.push(band)}
 const collarMat=mat(T,0x8b9498,.32,.72),collars=[];for(const y of[.34,.67]){const collar=mesh(T,new T.CylinderGeometry(.035,.035,.032,24),collarMat);collar.position.y=y;shaftGroup.add(collar);collars.push(collar)}
 ...
 function render(){shaft.scale.y=p.targetHeight;shaft.position.y=p.targetHeight/2;bands.forEach((band,i)=>{band.position.y=(i+.5)*.2;band.visible=band.position.y<p.targetHeight-.02});collars[0].position.y=p.targetHeight*.34;collars[1].position.y=p.targetHeight*.67;prism.position.y=p.targetHeight;poleRoot.rotation.z=-p.poleTiltRad;person.scale.setScalar(p.personHeight/1.7);if(!context.reducedMotion)person.position.y=Math.sin(time*1.3)*.004;else person.position.y=0}
