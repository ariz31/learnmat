...
export function createAsset(context={}){
 ...
 const handle=mesh(T,new T.BoxGeometry(.28,.055,.06),dark);handle.position.set(0,.22,.02);instrument.add(handle);
 ...
 const focusKnob=mesh(T,new T.CylinderGeometry(.07,.07,.08,24),dark);focusKnob.rotation.z=PI/2;focusKnob.position.set(.20,.09,.08);instrument.add(focusKnob);
 const tangent=mesh(T,new T.CylinderGeometry(.025,.025,.16,18),dark);tangent.rotation.z=PI/2;tangent.position.set(-.24,-.12,.17);instrument.add(tangent);
 const objectiveRing=mesh(T,new T.TorusGeometry(.102,.012,10,32),aluminum);objectiveRing.position.set(0,0,-.468);telescope.add(objectiveRing);
 ...
 function render(){
   ...
   for(let i=0;i<3;i++){const a=-PI/2+i*2*PI/3,foot=new T.Vector3(p.legSpread*Math.cos(a),0,p.legSpread*Math.sin(a)),upper=new T.Vector3(0,topY-.05,0),lower=new T.Vector3(foot.x,.10,foot.z),mid=upper.clone().lerp(lower,.58),v=lower.clone().sub(upper).normalize();placeCylinder(T,legs[i],upper,mid);placeCylinder(T,innerLegs[i],mid,lower);placeCylinder(T,clamps[i],mid.clone().addScaledVector(v,-.04),mid.clone().addScaledVector(v,.04));feet[i].position.set(foot.x,.09,foot.z);feet[i].quaternion.setFromUnitVectors(new T.Vector3(0,1,0),v)}
