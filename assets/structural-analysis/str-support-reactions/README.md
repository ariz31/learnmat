...
Version 0.3.1 retains the 0.3.0 projected leader system, which replaced the former free-floating label sprites with projected screen-space leader annotations. The component now exposes semantic 3D annotation anchors in `snapshot().annotations`; the host demo projects those anchors through the active camera.
